...
{
	//Individuals move amoungst popualtions, are coppied from pop1 to pop2
	double random;
	...
	for(int p=0;p<nPop;p++)
	{
		if(p==0)//left most population
		{
			for(int i=0;i<nInd1[p];i++)
			{
				...
				{
					//cout<<"Indiviudal moves 0->1"<<endl;
					for(int c=0;c<2;c++)
					{
		...
		else if(p==nPop-1) //right most popuatlion
		{
			for(int i=0;i<nInd1[p];i++)
			{
				if(rand()/(double)RAND_MAX<m/2.0)//Moves left
				{
					//cout<<"Indiviudal moves  2->1"<<endl;
					for(int c=0;c<2;c++)
					{
						for(int l=0;l<nHybrid;l++) pop2[p-1][nInd2[p-1]].hLoci[c][l]=pop1[p][i].hLoci[c][l];
						for(int l=0;l<nOther;l++) pop2[p-1][nInd2[p-1]].oLoci[c][l]=pop1[p][i].oLoci[c][l];
		...
		else //migration both directions
		{
			for(int i=0;i<nInd1[p];i++)
			{
				...
				{
					//cout<<"Indiviudal moves  1->0"<<endl;
					for(int c=0;c<2;c++)
					{
						for(int l=0;l<nHybrid;l++) pop2[p-1][nInd2[p-1]].hLoci[c][l]=pop1[p][i].hLoci[c][l];
						for(int l=0;l<nOther;l++) pop2[p-1][nInd2[p-1]].oLoci[c][l]=pop1[p][i].oLoci[c][l];
				...
				{
					//cout<<"Indiviudal moves  1->2"<<endl;
					for(int c=0;c<2;c++)
					{
						for(int l=0;l<nHybrid;l++) pop2[p+1][nInd2[p+1]].hLoci[c][l]=pop1[p][i].hLoci[c][l];
						for(int l=0;l<nOther;l++) pop2[p+1][nInd2[p+1]].oLoci[c][l]=pop1[p][i].oLoci[c][l];
...
void parsSet::mating()
{
	...
	for(int p=0;p<nPop;p++)
	{
		lambda=1+R*(1.0-(double)nInd2[p]/K);
		for(int i=0;i<nInd2[p];i++)
		{
			for(int off=0;off<poisson(lambda);off++)
			{
				//gemerating parent 1 gamete
				chromo=rand()%2;
